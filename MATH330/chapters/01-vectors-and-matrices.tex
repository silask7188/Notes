...
%    Let $A$ be an $m \times n$ matrix with column vectors $\vct{a}_1, \vct{a}_2, \dots, \vct{a}_n \in \R^m$. The \textbf{column space} of $A$, denoted $\col(A)$, is the set of all linear combinations of the columns of $A$:
%    \[
%        \col(A) \coloneqq \spn\{\vct{a}_1, \vct{a}_2, \dots, \vct{a}_n\} = \set{A\vct{x}}{\vct{x} \in \R^n} \subseteq \R^m
%    \]
%\end{definition}

%The column space $\col(A)$ consists of all vectors $\vct{b} \in \R^m$ for which the linear system $A\vct{x} = \vct{b}$ has at least one solution $\vct{x} \in \R^n$.
